...
插件先按 DOI（标题为后备）寻找 Zotero 条目，再提取整篇 PDF 文字和坐标，创建原生 PDF 高亮并附上译文和笔记，支持符合条件的相邻两页选区。内置 pdf-lib 在内存中分组页面，再通过 Zotero 原生识别器提取坐标并恢复原页序；不修改原 PDF。缓存随文件修改而失效；读取不完整或多处匹配时拒绝写入。
