...
1. 项目公开仓库为 [Translate-Bridge-for-Zotero](https://github.com/jinwenpang0523-alt/Translate-Bridge-for-Zotero)。
2. 将项目源码提交并推送到新仓库。上传前检查 `git status` 和暂存区，不要上传私钥、个人配置、缓存或本地工作记录。项目 `.gitignore` 已排除这些已知路径；安装包在 `dist/`，作为 Release 附件分发。
3. 在仓库 **Releases → Draft a new release** 创建版本，标签用 `v0.7.0`，标题用 `Translate Bridge for Zotero 0.7.0`，说明复制本目录 `RELEASE-0.7.0.md`。
4. 附加 `dist/translate-bridge-for-zotero-0.7.0.zip`。也可同时附加独立的 Chrome ZIP 与 Zotero XPI。
